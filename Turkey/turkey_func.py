...
def make_camel(x):
    ...
    no_camel_list=['and','to','in','with','(except','reserves)','(current','(long-term','assets)',
                   'for','from','of']
    
    abb_list=['Fx','Sdrs','Imf','Zar','Cbrt','Sdr']
    
    sx=[i if i in no_camel_list else i.title() for i in sx]
    
    sx=[i.replace('Sdrs','Sdr').upper() if i in abb_list else i for i in sx]
    
    nx=' '.join(sx).replace('Where Investment Risk Belongs to Ph','where investment risk belongs to ph')
...
def create_data_dic(key_list,username,password,url_try,url_usd,sub_url_list,defined_text_w):
    ...
    for i in range(len(key_list)):
        ...
        try_data=get_data(username=username,password=password,url=url_try.format(sub_url_list[i]))

        usd_data=get_data(username=username,password=password,url=url_usd.format(sub_url_list[i]))
        
        if key_list=='ex_by_issuer':
            
            try_data=specific_augment_data(try_data)
            
            usd_data=specific_augment_data(usd_data)
            
        else:
            
            try_data=augment_data(try_data)
            
            usd_data=augment_data(usd_data)

        data_dic['exe_sum'][key_list[i]]={}

        data_dic['exe_sum'][key_list[i]]['date']=month_year_extract(try_data.iloc[0]['date'])

        data_dic['exe_sum'][key_list[i]]['usd']=manipulate_num(usd_data.iloc[0]['total'])

        data_dic['exe_sum'][key_list[i]]['try']=manipulate_num(try_data.iloc[0]['total'])
    
        data_dic[key_list[i]]={}

        data_dic[key_list[i]]['usd']={}

        data_dic[key_list[i]]['try']={}
        ...
        try_col_name=list(try_data.columns)

        for j in range(len(try_col_name)):

            temp_name_usd=usd_col_name[j].replace(',','').replace(' - ','_').replace(' ','_').replace('-','_').replace('(','').replace(')','')

            temp_name_try=try_col_name[j].replace(',','').replace(' - ','_').replace(' ','_').replace('-','_').replace('(','').replace(')','')
            ...
            if try_col_name[j]=='date':

                data_dic[key_list[i]]['try'][temp_name_try]=[month_year_extract(try_data.iloc[k][try_col_name[j]]) for k in range(5)]

            else:

                temp_name_list.append(textwrap.fill(make_camel(try_col_name[j]),defined_text_w).replace('\n','\\\\'))

                data_dic[key_list[i]]['try'][temp_name_try]=[manipulate_num(try_data.iloc[k][try_col_name[j]]) for k in range(5)]

                temp_name_list2.append(temp_name_try)
