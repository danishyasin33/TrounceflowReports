...
def create_data_dic(key_list,username,password,url_zar,url_usd,sub_url_list,defined_text_w):
    ...
    for i in range(len(key_list)):
        ...
        zar_data=get_data(username=username,password=password,url=url_zar.format(sub_url_list[i]))

        usd_data=get_data(username=username,password=password,url=url_usd.format(sub_url_list[i]))
        
        if key_list=='ex_by_issuer':
            
            zar_data=specific_augment_data(zar_data)
            
            usd_data=specific_augment_data(usd_data)
            
        else:
            
            zar_data=augment_data(zar_data)
            
            usd_data=augment_data(usd_data)

        data_dic['exe_sum'][key_list[i]]={}

        data_dic['exe_sum'][key_list[i]]['date']=month_year_extract(zar_data.iloc[0]['date'])

        data_dic['exe_sum'][key_list[i]]['usd']=manipulate_num(usd_data.iloc[0]['total'])

        data_dic['exe_sum'][key_list[i]]['zar']=manipulate_num(zar_data.iloc[0]['total'])
    
        data_dic[key_list[i]]={}

        data_dic[key_list[i]]['usd']={}

        data_dic[key_list[i]]['zar']={}
        ...
        zar_col_name=list(zar_data.columns)

        for j in range(len(zar_col_name)):

            temp_name_usd=usd_col_name[j].replace(',','').replace(' - ','_').replace(' ','_').replace('-','_').replace('(','').replace(')','')

            temp_name_zar=zar_col_name[j].replace(',','').replace(' - ','_').replace(' ','_').replace('-','_').replace('(','').replace(')','')
            ...
            if zar_col_name[j]=='date':

                data_dic[key_list[i]]['zar'][temp_name_zar]=[month_year_extract(zar_data.iloc[k][zar_col_name[j]]) for k in range(5)]

            else:

                temp_name_list.append(textwrap.fill(make_camel(zar_col_name[j]),defined_text_w).replace('\n','\\\\'))

                data_dic[key_list[i]]['zar'][temp_name_zar]=[manipulate_num(zar_data.iloc[k][zar_col_name[j]]) for k in range(5)]

                temp_name_list2.append(temp_name_zar)
